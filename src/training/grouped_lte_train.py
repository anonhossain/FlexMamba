...
from src.evaluation.grouped_lte_eval import (
    evaluate_grouped_lte,
)
...
CONFIG_PATH = "src/configs/techniques/s07_grouped_lte.yaml"
...
def find_parent_checkpoint(
    ...
    if cfg["parent"]["technique"] != "dynamic_mor":
        raise ValueError(
            "S07 parent must be dynamic_mor."
        )
...
def train_grouped_lte(
    ...
    parent_checkpoint = find_parent_checkpoint(
        cfg,
        parent_run_dir,
    )
    ...
    print(
        "Source parent parameters: "
        f"{init_report['source_parent_parameters']:,}"
    )
    print(
    ...
    print(
        "Grouped parameter savings: "
        f"{init_report['grouped_parameter_savings']:,}"
    )
    print(
        "New LTE router parameters: "
        f"{init_report['new_lte_router_parameters']:,}"
    )
    print(
        "Parent-derived fraction: "
        f"{init_report['parent_derived_fraction']:.4f}"
    )
    ...
    # Combined architecture changed -> fresh optimizer.
    ...
    try:
        ...
        summary["initialization_report"] = (
            init_report
        )

        summary["grouping_report"] = (
            model.grouping_report()
        )
        ...
        evaluation = evaluate_grouped_lte(
            summary
        )
...
def run_grouped_lte(
    ...
    summary, evaluation = train_grouped_lte(
    ...
    print(
        "Average active bins: "
        f"{evaluation['lte']['average_active_bins']:.3f}"
    )
