...
def run_evaluation(model, batches, device):
    ...
    for batch in batches:
        ...
        last_state_report = output["state_report"]
    ...
    return {
        ...
        "state": last_state_report,
    }


def evaluate_recursion_wise_state(summary):
    ...
    if cfg["technique"]["model_type"] != "recursion_wise_state_mamba":
        raise ValueError("Checkpoint is not Recursion-Wise State Mamba.")
    ...
    print("\nRecursion-Wise State evaluation completed.")
    print(f"NLL: {report['quality']['nll']:.4f}")
    print(f"PPL: {report['quality']['perplexity']:.2f}")
    print(f"State banks: {report['state']['state_banks']}")
    print(f"State memory: {report['state']['theoretical_shared_state_memory_mb']:.3f} MB")
