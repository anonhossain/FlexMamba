...
from transformers import MambaConfig as HFMambaConfig, MambaForCausalLM
...
class RecursiveMambaConfig:
    ...
    def __post_init__(self):
        if self.input_layers < 1:
            raise ValueError("input_layers must be >= 1")
        if self.shared_middle_layers < 1:
            raise ValueError("shared_middle_layers must be >= 1")
        if self.num_recursions < 1:
            raise ValueError("num_recursions must be >= 1")
        if self.output_layers < 1:
            raise ValueError("output_layers must be >= 1")
...
class RecursiveMambaCausalLM(nn.Module):
    ...
    Physical:
        Input → Shared Blocks → Output

    Effective:
        Input → [Shared Blocks × Recursions] → Output
    """

    def __init__(self, config: RecursiveMambaConfig):
        super().__init__()
        self.cfg = config
    ...
    def forward(self, input_ids, labels=None):
        ...
        return {
            ...
            "loss": lm_loss,
            "lm_loss": lm_loss,
            "avg_depth": recursion_depth,
            "avg_recursion_depth": recursion_depth,
        }
    ...
    def load_parent_state_dict(
        ...
        if strategy != "cycle_average":
            raise ValueError(
                "Recursive-Mamba currently supports only "
                "strategy='cycle_average'."
            )

        parent_layers = parent_config["model"]["n_layers"]
        expected_layers = self.cfg.effective_layers

        if parent_layers != expected_layers:
            raise ValueError(
                f"Parent depth does not match Recursive-Mamba effective depth. "
                f"Parent={parent_layers}, Recursive={expected_layers}"
            )
        ...
        for key, value in target_state.items():
            ...
            if key in parent_state and parent_state[key].shape == value.shape:
            ...
            for target_key in target_state:
                ...
                for source_layer in source_layers:
                    ...
                    if source_key not in parent_state:
                        raise KeyError(f"Missing parent key: {source_key}")
                ...
                if first.dtype.is_floating_point:
                    mapped_value = torch.stack(
                        [tensor.float() for tensor in source_values]
                    ).mean(dim=0).to(first.dtype)
                else:
                    mapped_value = first.clone()

                mapped_state[target_key] = mapped_value
        ...
        for j in range(self.cfg.shared_middle_layers):
            ...
            target_layer = self.cfg.input_layers + j
            map_layer(target_layer, source_layers)
        ...
        target_output_start = self.cfg.input_layers + self.cfg.shared_middle_layers
        source_output_start = self.cfg.effective_layers - self.cfg.output_layers

        for j in range(self.cfg.output_layers):
            map_layer(
                target_output_start + j,
                [source_output_start + j],
            )
        ...
        loaded_elements = sum(t.numel() for t in mapped_state.values())
        total_elements = sum(t.numel() for t in target_state.values())

        return {
            "strategy": strategy,
            "loaded_tensor_count": len(mapped_state),
            "loaded_parameter_fraction": loaded_elements / total_elements,
